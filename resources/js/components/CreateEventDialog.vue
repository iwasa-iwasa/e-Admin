...
const MAX_FILE_SIZE = 41943040 // Approximately 40MB
...
const getDefaultTimes = () => {
  ...
  const startHour = jstTime.getHours() + 1
...
const form = useForm({
  ...
  start_time: getDefaultTimes().startTime,
  end_time: getDefaultTimes().endTime,
...
const handleSave = () => {
  ...
  const transformData = (data: any) => {
    ...
    const transformed: Record<string, any> = {
      ...data,
      date_range: [
        data.date_range[0].toLocaleDateString('sv-SE'),
        data.date_range[1].toLocaleDateString('sv-SE')
      ],
...
<template>
  <Dialog :open="open" @update:open="(val) => { if (!val) resetForm(); emit('update:open', val) }">
    <DialogContent class="max-w-3xl md:max-w-4xl lg:max-w-5xl w-[95vw] md:w-[66vw] max-h-[90vh]">
      <DialogHeader>
        <DialogTitle>
          {{ isEditMode ? (canEdit ? '予定編集' : '予定確認') : '新規予定作成' }}
        </DialogTitle>
        <DialogDescription>
          {{ isEditMode ? (canEdit ? '部署内共有カレンダーの予定を編集します' : '部署内共有カレンダーの予定を確認します') : '部署内共有カレンダーに予定を追加します' }}
        </DialogDescription>
      </DialogHeader>
                ...
                <div class="space-y-4 pl-6 border-l-2 border-gray-300 ml-3">
                  ...
                  <div class="space-y-2">
                    ...
                    <Input 
                      ...
                      type="number" 
                      min="1" 
                      v-model.number="form.recurrence.recurrence_interval" 
                      :disabled="!canEdit" 
                      class="w-24" 
                    />
                  </div>
